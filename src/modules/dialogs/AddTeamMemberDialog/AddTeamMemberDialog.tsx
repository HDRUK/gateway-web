...
const AddTeamMemberDialog = () => {
    ...
    const { control, handleSubmit, getValues } = useForm<AddTeamMember>({
        // resolver: yupResolver(addTeamMemberValidationSchema),
        // defaultValues: { ...addTeamMemberDefaultValues },
    });

    const { data: teamUserList = [], isLoading: isTeamListLoading } =
    ...
    useEffect(() => {
    }, [memberSearchName, teamUserList]);

    return (
        <Dialog title={t("dialogs.AddTeamMemberDialog.title")}>
            <MuiDialogContent>
                <Typography>Users that you want to add to your team must already have an account on the Gateway</Typography>
                    <Box sx={{
                        display: "flex",
            ...
            <MuiDialogActions>
                <ModalButtons
                    confirmText={t("dialogs.AddTeamMemberDialog.confirmButton") || ""}
                    confirmType="submit"
                    cancelText={t("dialogs.AddTeamMemberDialog.cancelButton") || ""}
                />
            </MuiDialogActions>
        </Dialog>
    );
