...
code = code
    ...
    .replace(
        ...
        'from "https://esm.sh/@mui/material@5.14.18?deps=react@18"'
    )
    .replace(
        /from\s*["']@mui\/material\/styles["']/g,
        'from "https://esm.sh/@mui/material@5.14.18/styles?deps=react@18"'
    )
    .replace(
        /from\s*["']@mui\/material\/([A-Za-z0-9_-]+)["']/g,
        'from "https://esm.sh/@mui/material@5.14.18/$1?deps=react@18"'
    )
    .replace(
        /from\s*["']@mui\/icons-material\/([^"']+)["']/g,
        (_m, sub) =>
            `from "https://esm.sh/@mui/icons-material@5.14.18/${sub.replace(
                /\.js$/,
                ""
            )}?bundle&deps=react@18,@mui/material@5.14.18"`
    )

    // emotion
    .replace(
        /from\s*["']@emotion\/react["']/g,
        'from "https://esm.sh/@emotion@11.11.0/react@11?deps=react@18"'
    )
