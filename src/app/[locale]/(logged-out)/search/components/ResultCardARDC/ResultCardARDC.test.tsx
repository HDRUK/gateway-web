...
describe("ResultCardARDC", () => {
    ...
    it("does not render a logo img when providerLogo is undefined", () => {
        ...
        expect(screen.queryByRole("img", { name: "ARDC" })).not.toBeInTheDocument();
    });
    ...
    it("link href uses slug when present", () => {
        render(<ResultCardARDC result={mockResult} />);
        expect(screen.getByRole("link")).toHaveAttribute(
            "href",
            "https://researchdata.edu.au/datasets/test-dataset"
        );
    });

    it("link href falls back to ARDC base URL when slug is absent", () => {
        render(<ResultCardARDC result={{ ...mockResult, slug: "" }} />);
        expect(screen.getByRole("link")).toHaveAttribute(
            "href",
