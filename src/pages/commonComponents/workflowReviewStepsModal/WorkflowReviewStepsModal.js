...
const WorkflowReviewStepsModal = ({ open, close, workflow = {} }) => {
        ...
        if (!_.isEmpty(steps)) {
            return [...steps].reduce((arr, step) => {
                if (!_.isEmpty(step)) {
                    ...
                    const { reviewers, recommendations, _id, active } = step;
    ...
    const buildReviews = (stepId = '', reviewers = [], recommendations = []) => {
        if (!_.isEmpty(reviewers)) {
            return [...reviewers].map(rev => {
                ...
                const review = recommendations.find(r => r.reviewer === rev._id) || {};
    ...
    const toggleStep = (step = {}) => {
        if (!_.isEmpty(workflowObj) && !_.isEmpty(step)) {
            const steps = updateStepToggle([...workflowObj.steps], step);
            const workflow = {
                ...workflowObj,
                steps,
            };
    ...
    const setToggleReview = (review = {}) => {
        const { steps } = workflowObj;
        const modifiedSteps = [...steps].reduce((arr, step) => {
            let modifiedReviews = [];
            const { reviews } = { ...step };
