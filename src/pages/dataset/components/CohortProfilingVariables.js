...
const CohortProfilingVariables = props => {
    ...
    const searchVariableData = async e => {
        if (e.key === 'Enter') {
            ...
            await axios
                .get(
                    `${baseURL}/api/v1/cohortProfiling/${props.datasetID}/${props.tableName}/${cohortProfilingVariables.label}?value=${searchTerm}`
                )
                .then(res => {
                    setCohortProfilingVariableValues(res.data.cohortProfiling[0].values);
    ...
    const sortVariableData = async paramSortType => {
        ...
        await axios
            .get(
                `${baseURL}/api/v1/cohortProfiling/${props.datasetID}/${props.tableName}/${cohortProfilingVariables.label}?sort=${paramSortType}${searchString}`
            )
            .then(res => {
                setCohortProfilingVariableValues(res.data.cohortProfiling[0].values);
